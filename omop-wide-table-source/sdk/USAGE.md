...
| `aggregate(cohort=None, *, gender_male=8507, gender_female=8532, source_field="source_field_name", max_workers=1, progress=False)` | `dict[str, polars.DataFrame]` | Wide-format OMOP aggregate, one frame per dataset. `source_field` selects whether field names or descriptions are preferred as headers, with the other as fallback. Honors the client's SQLAlchemy/ADBC driver and always returns polars. |
