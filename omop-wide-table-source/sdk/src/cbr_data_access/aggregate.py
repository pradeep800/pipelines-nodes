...
def _spool_long_parquet(
    ...
    *,
    ...
    Fetches with a plain forward-only cursor (proxy-safe; the proxy rejects
    server-side cursors with "cursor can only scan forward, 55000") and writes
    one zstd-compressed Parquet part file per fetch batch into a temporary
    directory. Returns the directory path; on success the CALLER owns it and
    must delete it, on any error it is removed here before re-raising.
            ...
            while True:
                ...
                if rows - last_report >= 500_000:
                    ...
                    report(
                        ...
                        f"rows={rows} seconds={time.perf_counter() - spool_start:.1f}"
                    )
            if parts == 0:
...
def _finalize_dataset_frames(
    ...
    for name, df in datasets.items():
        ...
        finalized[name] = _select_wide_columns(_sort_wide_rows(df), index_cols)
        report(
...
def _aggregate_cohorts(
    ...
    *,
    ...
    report(f"aggregate start cohorts={cohorts} workers={worker_count}")
    datasets: dict[str, pl.DataFrame] = {}
